...
## 批次 6（2026-09-22 派发，**最后一轮**）—— 路线 B 收尾与冻结前总检

> 背景：2026-09-22 当天完成路线 B（BSM2 整厂）、两块补强（工况条件化/对拍差分、标定漂移专项）、跨相位稳健性、漂移扩展、冻结前数字总检，写入 PROJECT_STATE 第 27–30 节。本批全部为独立复核，**禁止 import src/dsh/route_b_common.py**。**六个 Q 全部必做**。

### Q24 冻结前数字总检的独立复算（必做，最高优先）
- 读：PROJECT_STATE.md 第 27–30 节；results/2026-09-22/dsh/freeze_check.{json,md}；src/dsh/2026-09-22_08_freeze_check.py（只读）
- 做：
  ① 自己写脚本，**从结果文件独立复算**下列数字并核对：路线 B（真值失效 109.00、q999 阈值 13.149、零误报阈值 19.560、原始量零误报延迟 86.11、原始量首报 146.11、比值通道含噪中位 9.80 与区间 5.03–11.03、提前量 37.47、RUL 1 天窗 26.47 / 10 天窗 13.85、18% 终值增幅 55.53%、120 天斜坡实际终值 19.67）、补强①（对拍差分 13.37）、补强②（随机游走 3%/5% = 46.99/72.72、双测量平均 30.03）、污泥线（109.00 / 61.33 / 47.67 / 1.00）；
  ② **独立核对文档一致性**：这些数字是否出现在 PROJECT_STATE 与 docs/07、08、09、演示页里（注意同一数值可能有多种书写）；
  ③ 给出你自己的检查清单：**你认为还应加进总检、但我们没检查的项目**（例如演示页的图注、docs/09 的行、四图里的标注）。
- 产物：results/2026-09-22/codex_b6/freeze_recheck.md
- 验收：逐项「一致/不一致 + 你的复算值 + 来源文件」；不一致要给复现命令。

### Q27 标定漂移专项复核（必做）
- 读：PROJECT_STATE.md 第 29.2 / 30.2 节；results/2026-09-22/dsh/bsm2_route_b_drift.csv、bsm2_route_b_drift_ext.csv、bsm2_route_b_pairdiff_seeds.csv；src/dsh/2026-09-22_06_*.py 与 _09_*.py（只读）
- 做：自己实现「比值通道 + 零误报阈值 + 2% 仪表噪声 + 3% 日粒度实验室噪声」，复核六类结构（线性慢漂移 / 阶跃 / 周期 6.3 天 / 周期 30 天 / **无界随机游走** / **AR(1) 均值回归**）在 1%/3%/5%/10% 下的延迟与检出率；**重点检验我们标为「假设」的两条**：
  ① 「双测量平均（两路独立漂移取均值）对**无界随机游走**有效（46.99 → 30.03 天）」；
  ② 「双测量平均对**均值回归的 AR(1)** 无效（20.55 → 20.13 天），因为阈值与信号侧的漂移贡献部分抵消」。
  如果你得到不同结论，请给出你的漂移实现与阈值口径。
- 产物：results/2026-09-22/codex_b6/drift_recheck.md
- 验收：结构表逐项；两条假设各给「支持/不支持 + 你的数字 + 机理说明」。

### Q25 跨相位稳健性复核（必做）
- 读：PROJECT_STATE.md 第 30.1 节；results/2026-09-22/dsh/bsm2_route_b_seasons.{csv,md}、season_<相位>_{healthy,degraded}.csv.gz、seasons_part*.json
- 做：在官方 609 天动态进水的**第 0 / 200 / 400 天起**三个相位上，各跑健康与退化两条 160 天整厂轨迹（你自己的实现或官方 BSM2Base），核对三条：
  ① 真值失效是否都 = 第 109.00 天（退化在设备侧注入，与进水相位无关）；
  ② 原始量（5 个可测量、零误报口径）是否都**晚于失效**（我们：86.11 / 81.58 / 81.16 天）；
  ③ 设备级比值通道含噪延迟中位是否都在 ~10 天（我们三个相位都是 9.80 天，区间 5.03–10.85）。
- 产物：results/2026-09-22/codex_b6/seasons_recheck.md
- 验收：三条逐相位；不一致给证据。

### Q26 补强①复核（工况条件化 + 同工况对拍差分）（必做）
- 读：PROJECT_STATE.md 第 29.1 / 30.3 节；results/2026-09-22/dsh/bsm2_route_b_cond.{csv,json}、bsm2_route_b_pairdiff.csv、bsm2_route_b_pairdiff_seeds.csv
- 做：
  ① 自己实现「冻结参考 + 进水流量分位箱（4 与 8 箱）× 小时」条件化，核对健康轨迹最高分（我们 14.86 / 15.59，无条件 19.18）与「**4 箱下退化仍在第 66.16 天越过阈值，属抢跑**」这一判断（越过幅度 0.30，而故障当刻只贡献约 +1.5% 湿泥饼量）；
  ② 自己实现「同工况对拍差分」（配对差分 + **日尺度平滑** + 零分布由健康-健康对拍实测），核对比值通道的 13.37 天（8 对）与样本量敏感性（4/8/12 对 → 12.88 / 13.36 / 15.07 天）；
  ③ 特别核对：**不做日尺度平滑时**，单点刀锋尖峰会不会把零分布最大值推到 16+（我们踩过这个坑）。
- 产物：results/2026-09-22/codex_b6/conditioned_recheck.md
- 验收：①②③ 逐条；不一致给你的实现细节。

### Q28 静态审计 + 最终冻结建议（必做）
- 读：PROJECT_STATE.md 第 27–30 节、设计原则⑦、docs/07 行 9–11 与原则 7、docs/08 第 27/29 节行、docs/09 路线 B 行、demo/lanmai_demo.html 的「路线 B」与「补强」卡片、run_all.py（含新增 route_b_* 五个阶段）
- 做：
  ① 检查第 27–30 节的表述与数字是否自洽、有无过强全称量词、局限是否齐全；
  ② 「python run_all.py --full」的可运行性抽查（可只抽查新增的 5 个阶段 + demo + check 阶段；不必全跑完，但要说明你抽查了哪些、结果如何）；
  ③ ASCII 文件名、产物路径、脚本间依赖（route_b_common）是否有工程隐患；
  ④ **最终冻结建议**：给出一份「可冻结 / 需带限定 / 仍不确定」三栏清单（这项是本批最重要的交付）。
- 产物：results/2026-09-22/codex_b6/final_audit.md
- 验收：每项「有问题/无问题 + 证据（文件与行号）」；三栏清单必须完整。

### Q29 MetroPT-3 事件机口径自洽性检查（必做，历史小项）
- 读：PROJECT_STATE.md 第 16 节；docs/PROTOCOL_v1.5.md；handoff/2026-09-15_codex_to_dsh_r2.md 与 r14/r15 相关条目
- 做：只回答一个问题 —— **冻结数字（timely 2/4、误报 0.1253、TIA-H 11.6%）所依据的事件机口径，是否与协议文档和我们引用的表述自洽**（退出规则 0.8×thr、起点约定、冷却点数）；不需要重跑数据。
- 产物：results/2026-09-22/codex_b6/metropt3_consistency.md
- 验收：一句结论 + 依据（文档行号）；若有不自洽，指出应该改哪里。
