...
OUT='results/2026-09-20/dsh'; os.makedirs('demo',exist_ok=True)
...
SCEN=[
 ...
 ('deg20','慢退化 -20% / 100 天','曝气能力 100 天缓慢衰减 20%：出水指标几乎不变，模型分数开始抬升；第 110 天才越线，提前量 70 天。','bsm1_120d_baseline.csv','bsm1_sweep_keep80_ramp100.csv',40.28,110.34),
 ('deg40','慢退化 -40% / 100 天（最完整一例）','第 20 天开始退化 → 第 40 天报警 → 第 47 天出水指标真实失效。机理-溶解氧外推剩余寿命 3.24 天，真值 7.27 天，误差 4.04 天。','bsm1_120d_baseline.csv','bsm1_120d_degraded.csv',40.28,47.34),
...
for sid,title,note,fb,fd,alarm,fail in SCEN:
    ...
    X=D=prep(pd.read_csv(os.path.join(OUT,fd))) if fd else B
    ...
    ax[0].plot(X.t_day,X.SO3,color=BL,lw=1.1,label='出水 SO3（mg/L）')
    ax[0].axhline(0.5,color=RD,ls='--',lw=1.1)
    ax[0].text(1,0.56,'失效线 SO3=0.5',color=RD,fontsize=8)
...
for k,(sid,title,note,fb,fd,alarm,fail) in enumerate(SCEN):
    a=ax[k//3][k%3]
    B=prep(pd.read_csv(os.path.join(OUT,fb))); X=prep(pd.read_csv(os.path.join(OUT,fd))) if fd else B
...
html=('<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><title>澜脉 · 数字孪生效果（静态版）</title><style>'
 ...
 '<div class="sub">IWA BSM1 仿真，120 天，设备=曝气系统。上曲线：出水 SO3 与曝气能力 KLa；下曲线：模型健康分数与判据阈值。黄色竖线=模型报警，红色竖线=真实失效。'
