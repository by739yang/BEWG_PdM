...
ax[0].plot(x,S['提前量DO0_5'],'s--',color=RD,label='失效前提前量（SO3<0.5）')
ax[0].plot(x,S['提前量DO1_0'],'^:',color=GO,label='DO 劣化前提前量（SO3<1.0）')
...
W('失效（反应池溶解氧 DO₃ 持续 1 天 < 0.5 mg/L）前提前量：'+'、'.join('%s %s 天'%(a,b) for a,b in zip(S['最终KLa比例']*100, S['提前量DO0_5']))+'。'+NL)
W('出水劣化（SO3 持续 1 天 < 1.0 mg/L）前提前量：'+'、'.join('%s %s 天'%(a,b) for a,b in zip(S['最终KLa比例']*100, S['提前量DO1_0']))+'。'+NL)
