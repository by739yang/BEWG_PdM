...
ax[0].plot(x,S['提前量SO3_0_5'],'s--',color=RD,label='失效前提前量（DO₃<0.5）')
ax[0].plot(x,S['提前量SO3_1_0'],'^:',color=GO,label='DO₃ 劣化前提前量（<1.0）')
...
W('失效（反应池溶解氧 DO₃ 持续 1 天 < 0.5 mg/L）前提前量：'+'、'.join('%s %s 天'%(a,b) for a,b in zip(S['最终KLa比例']*100, S['提前量SO3_0_5']))+'。'+NL)
W('反应池溶解氧 DO₃ 持续 1 天 < 1.0 mg/L 前提前量：'+'、'.join('%s %s 天'%(a,b) for a,b in zip(S['最终KLa比例']*100, S['提前量SO3_1_0']))+'。'+NL)
W('**对 -40%/100 天及更快的退化，报警不早于曝气功能劣化**（提前量为负），只有失效前数天的提前量；退化越慢，失效前提前量越大，但检出延迟基本固定在 20 天左右（0.2-0.6%/天）或 7-14 天（0.8-2.0%/天）。'+NL)
W('## 5. 第四条设计原则'+NL)
