    ...
    for t in range(n):
        if state==0:
            if over[t]:
                if run==0: start=t
                run+=1
                if run>=CI and t-last_end>COOL: state=1; run=0
            else: run=0
        else:
            if (not over[t]) or sc[t]<RATIO*thr:
                run+=1
...
for f in files:
    ...
    for name,sc in scores.items():
        ...
        for q in QUANTS:
            ...
            det.append(dict(file=os.path.relpath(f,BASE),method=name,quantile=q,recall=rr['recall'],
                            fp_per_hour=rr['fp_per_hour'],tia=rr['tia'],delay=rr['delay_median']))
    print('done',os.path.basename(f),flush=True)
df=pd.DataFrame(rows); dd=pd.DataFrame(det)
df.to_csv(os.path.join(OUT,'protocol_v11_per_file.csv'),index=False,encoding='utf-8-sig')
dd.to_csv(os.path.join(OUT,'protocol_v11_det.csv'),index=False,encoding='utf-8-sig')
...
open(os.path.join(OUT,'protocol_v11_summary.md'),'w',encoding='utf-8').write(
