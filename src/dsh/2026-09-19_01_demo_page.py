...
h.append('<div class="warn"><b>结论：两个判据互补，不能混用话术。</b>比值判据（滚动 5 天中位数 / 该工况健康基线）在 12 个工况的健康运行上越限时长全为 0.000，而单点事件机是 0/4/3/26/18/26 次；但灵敏度差很多——k=1.3 只检出 9/12、延迟 25.8-93.7 天。因此：<b>单点事件机做即时报警</b>（阈值按工况基线标定），<b>比值判据做零误报的慢漂移确认与严重度分级</b>，不得把比值判据说成早期预警。</div>')
h.append('<h2 style="margin-top:16px">组合报警策略：上报用并集、派工用与门（12 个工况）</h2>')
h.append('<img src="data:image/png;base64,%s">'%asimg)
h.append(tbl(asum))
h.append('<div class="warn"><b>怎么读这张表：</b>① 误报侧——比值判据在 12 个工况健康运行上 0 次，与门把总上报从 77 砍到 26（雨/暴雨工况仍有 10/6/8 次），与门归不了零；② 检出侧——并集与单点一致（12/12），与门只剩 7/12，四档慢漂移全漏；③ 因此推荐 <b>上报用并集、派工用与门分级</b>：P1 直接派工、P2 复核排计划、P3 趋势观察。<b>禁止</b>把慢漂移场景里 20.3 天的单点首报当预警提前量宣传（其时刻由共同外部成分主导）。</div>')
