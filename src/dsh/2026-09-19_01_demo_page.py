...
h.append('<div class="warn"><b>这一节的价值在「负结果 + 解法」：</b>① 整厂尺度上 5 个原始可测量在固定参考域下<b>检不出</b>这次退化 —— 健康轨迹第 66 天因进水 +23.9% 同样越限，所谓「首报」是误报抢跑；② 换成设备级物料平衡比值（干固体产率 / 湿泥饼量 = 含固率，两者都可测）后<b>免疫工况漂移</b>，健康 0 误报、含噪 9.8 天检出；③ 该退化在沼气/能耗/出水上几乎不可见（±0.03%），说明这类故障只能靠设备级量发现。详见 PROJECT_STATE 第 27 节。</div>')
...
h.append('<li><b>⑦ 设备级退化要用设备自身的物料平衡比值监测：</b>整厂原始量会被进水工况漂移淹没（第 66 天进水 +23.9%，健康轨迹同刻分数同样越限）→ 用「干固体产率 / 湿泥饼量」（= 含固率）这类设备级比值，健康 0 误报、含噪 9.8 天检出。</li>')
h.append('</ul>')
